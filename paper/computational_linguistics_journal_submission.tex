...
Sentence embedding models are typically evaluated on sentence-level semantic textual similarity (STS) benchmarks \cite{Cer2017SemEval,Muennighoff2023MTEB}, where recent contrastive models like E5 \cite{Wang2022} and BGE \cite{Xiao2023} achieve state-of-the-art performance. However, an open question remains: \textit{does performance on sentence-level paraphrase detection transfer to document-level compositional semantic tasks}—where texts share high-level structure but differ completely in surface features?

We introduce \textbf{plot-level semantic similarity detection} as a challenging evaluation task that addresses this question. Unlike STS benchmarks that test lexical paraphrase, our task requires models to recognize narrative structure despite zero lexical overlap. Consider these examples:
